...
namespace CathedraProject.Services
{
    public static class ExcelManager
    {
        ...
        public static void ExportTeachers(string filename, List<Teacher> teachers)
        {
            Excel.Application ex = new Excel.Application();
            //Отобразить Excel
            ex.Visible = false;
            ...
            string[] headers = { "ФИО", "Должность", "Ставка", "Стаж", "Ученая степень", "Ученое звание", "Дата рождения", "Телефон", "Email" };
            for (int i = 0; i < headers.Length; i++)
            {
                sheet.Cells[1, i + 1] = headers[i];
            }
            ...
            {
                sheet.Cells[i + 2, 1] = teachers[i].FIO;
                sheet.Cells[i + 2, 2] = teachers[i].Post.Name;
                sheet.Cells[i + 2, 3] = teachers[i].Rate.ToString();
                sheet.Cells[i + 2, 4] = teachers[i].Experience.ToString();
                sheet.Cells[i + 2, 5] = teachers[i].AcademicDegree;
                sheet.Cells[i + 2, 6] = teachers[i].AcademicRank;
                sheet.Cells[i + 2, 7] = teachers[i].Birthday;
                sheet.Cells[i + 2, 8] = teachers[i].Phone;
                sheet.Cells[i + 2, 9] = teachers[i].Email;
            }
